"""
Per-role tuning for the SDK orchestrator.

Bash is intentionally NOT in any role's `allowed_tools`. Shell commands are governed
only by the scoped allow-rules in `.claude/settings.json` (pytest, git, ...), and
...
MAX_TRANSITIONS = 4 * CAP + 4  # hard safety stop against a runaway loop
